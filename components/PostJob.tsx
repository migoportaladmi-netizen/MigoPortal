...
const PostJob: React.FC<PostJobProps> = ({ user, onPost, onBack }) => {
  ...
  const handleGenerateAI = async () => {
    ...
    setLoading(true);
    try {
      // AI generation removed — returns empty scaffolding for manual fill
      const result = {
        description: `Role: ${formData.title} based in ${formData.location}. Please add the full description manually.`,
        requirements: [],
        responsibilities: []
      };
      setFormData(prev => ({
